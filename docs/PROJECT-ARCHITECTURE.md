...
In `v0.2.10`, every managed project must have a concrete lifecycle control definition based on a Windows `.bat` script. This script is the authoritative operational contract used by OrchFlow to control the project lifecycle.
...
- users should be able to explicitly reload one or more registered projects so OrchFlow can reread lifecycle scripts, refresh detected functions, and surface any configuration changes without relying on automatic filesystem watching
...
OrchFlow should not, in `v0.2.10`:
...
For `v0.2.10`, the selected backend persistence stack is `SQLite` with `SQLAlchemy` and `Alembic`.
