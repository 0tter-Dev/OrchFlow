...
In `v0.3.17`, every managed project must have a concrete lifecycle control definition based on a Windows `.bat` script. This script is the authoritative operational contract used by OrchFlow to control the project lifecycle.
...
OrchFlow should not, in `v0.3.17`:
...
For `v0.3.17`, the selected backend persistence stack is `SQLite` with `SQLAlchemy` and `Alembic`.
