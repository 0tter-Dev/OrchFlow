...
OrchFlow is currently in the `v0.3.2` implementation stage as of `2026-08-30`.
...
- AI-assisted onboarding is now being implemented around a LiteLLM-backed gateway rather than direct provider-specific integration. Status, gateway health, model discovery, and authorized context manifest boundaries are available through `GET /ai/status`, `GET /ai/gateway/health`, `GET /ai/models`, `POST /ai/context-manifests`, `GET /ai/context-manifests/{manifest_id}`, `orchflow ai status`, `orchflow ai health`, `orchflow ai models`, `orchflow ai manifest-create`, and `orchflow ai manifest-show`. They require an authenticated user, record audit events, and return safe gateway or context-authorization metadata without sending project file contents to models or executing prompts. The OrchFlow adapter remains responsible for proposal validation, review-driven file generation, and approved mapping persistence.
