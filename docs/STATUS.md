...
OrchFlow is currently in the `v0.2.12` implementation stage as of `2026-08-29`.
...
| Project registry | Register and persist project definitions | implemented | Existing `.bat` registration, ownership persistence and management, normalized action mappings, first-argument dispatch validation, preferred identifier auto-detection, partial registration, manual lifecycle mapping, explicit unconfigured decisions, and derived configuration health are implemented |
| Ideal lifecycle function model | Define expected `.bat` functions and project configuration health | in_progress | Fixed ideal functions, function states, preferred mapping detection, manual mapping, explicit unconfigured decisions, and derived complete/partial/blocked health are implemented; reload workflow, lifecycle execution gating, warning UI, and AI-assisted improvement remain upcoming |
...
- The project is now operating in `v0.2.12`, which consolidates the first real backend and web implementation milestones, runtime inspection refinement, admin and ownership management, LiteLLM dependency onboarding, documentation/versioning governance, CI plus contract hardening, the first ideal lifecycle function domain model, automatic lifecycle configuration health, and manual lifecycle configuration decisions on `main`.
...
- Project registry is now implemented for existing `.bat` onboarding, with ownership persistence and management, auditable lifecycle action mappings, registration-time validation for first-argument dispatch compatibility, automatic preferred-identifier detection, partial project registration, and manual lifecycle configuration updates exposed through API and CLI contracts.
- The ideal lifecycle function model now defines the initial fixed expected functions, preferred script identifiers, function configuration states, and derived project configuration health in code. Project responses now expose complete or partial health and function-level configuration states, including explicit `unconfigured` decisions. Reload, lifecycle execution gating, warning UI, and AI-assisted improvements remain upcoming.
