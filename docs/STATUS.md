...
OrchFlow is currently in the `v0.3.1` implementation stage as of `2026-08-30`.
...
| AI assistance adapter with LiteLLM gateway | Analyze project folders and help generate `.bat` scripts | in_progress | The authenticated OrchFlow-owned status, LiteLLM gateway health, model discovery boundaries, disabled-by-default configuration checks, API routes, CLI commands, and audit events are implemented; authorized context manifests, project analysis, proposal review, and file writes remain planned |
...
- The project is now operating in `v0.3.1`, which consolidates the first real backend and web implementation milestones, runtime inspection refinement, admin and ownership management, LiteLLM dependency onboarding, documentation/versioning governance, CI plus contract hardening, the first ideal lifecycle function domain model, automatic lifecycle configuration health, manual lifecycle configuration decisions, explicit project reload, configured-action lifecycle execution gating, web lifecycle configuration controls, and authenticated AI assistance status, gateway health, and model discovery on `main`.
...
- AI-assisted onboarding is now being implemented around a LiteLLM-backed gateway rather than direct provider-specific integration. The status, gateway health, and model discovery boundaries are available through `GET /ai/status`, `GET /ai/gateway/health`, `GET /ai/models`, `orchflow ai status`, `orchflow ai health`, and `orchflow ai models`. They require an authenticated user, record audit events, and return safe gateway information without sending project files or executing prompts. The OrchFlow adapter remains responsible for allowed project context, explicit authorization, proposal validation, review-driven file generation, and approved mapping persistence.
