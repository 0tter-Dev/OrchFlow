...
OrchFlow is currently in the `v0.2.11` implementation stage as of `2026-08-28`.
...
| Project registry | Register and persist project definitions | implemented | Existing `.bat` registration, ownership persistence and management, normalized action mappings, first-argument dispatch validation, preferred identifier auto-detection, partial registration, and derived configuration health are implemented |
| Ideal lifecycle function model | Define expected `.bat` functions and project configuration health | in_progress | Fixed ideal functions, function configuration states, automatic preferred mapping detection, and derived complete/partial/blocked health are implemented; manual unconfigured decisions, reload workflow, warning UI, and AI-assisted improvement remain upcoming |
...
- The project is now operating in `v0.2.11`, which consolidates the first real backend and web implementation milestones, runtime inspection refinement, admin and ownership management, LiteLLM dependency onboarding, documentation/versioning governance, CI plus contract hardening, the first ideal lifecycle function domain model, and automatic lifecycle configuration health on `main`.
