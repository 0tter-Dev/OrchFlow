...
1. `fix(web-health): stabilize health refresh UX`
...
2. `feat(web-ui): add accessible UI primitives and icons`
...
3. `docs(brand): define OrchFlow visual identity direction`
...
4. `docs(installer): outline Windows installer and release matrix`
...
- keep Windows `.bat` lifecycle scripts as the authoritative operational contract for managed projects in `v0.3.26`
