...
The next implementation sequence should keep improving OrchFlow's own startup and web operator experience through pull-request-sized increments.

1. `feat(runtime): add batch runtime inspection`

   Add authorized multi-project runtime inspection so the web operator surface can refresh visible project status efficiently.

   Expected version decision: patch bump to `0.3.21`.

2. `feat(web): redesign operator workspace`

   Replace the current bootstrap-style web presentation with a compact daily operator workspace using project navigation, action icons, runtime summaries, readiness warnings, simple `pt-BR` and `en-US` text dictionaries, and backend preferences.

   Expected version decision: patch bump to `0.3.22`.

3. `feat(registry): add project unlink workflow`

   Let users remove a project from OrchFlow without deleting the local project folder or lifecycle `.bat` file, with authorization, audit events, API, CLI, and web affordances.

   Expected version decision: patch bump to `0.3.23`.
...
- keep Windows `.bat` lifecycle scripts as the authoritative operational contract for managed projects in `v0.3.20`
