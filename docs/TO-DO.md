...
1. `feat(preferences): add user web preferences`

   Add backend-owned user preference storage for locale, project view mode, and global status refresh interval, then expose and consume it through authenticated operator surfaces.

   Expected version decision: patch bump to `0.3.20`.

2. `feat(runtime): add batch runtime inspection`
...
3. `feat(web): redesign operator workspace`
...
4. `feat(registry): add project unlink workflow`
