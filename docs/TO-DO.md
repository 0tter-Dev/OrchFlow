...
1. `feat(devx): add PID-based local process control`
...
2. `feat(web-auth): redesign unauthenticated login flow`
...
3. `fix(web-health): stabilize health refresh UX`
...
4. `feat(web-ui): add accessible UI primitives and icons`
...
5. `docs(brand): define OrchFlow visual identity direction`
...
6. `docs(installer): outline Windows installer and release matrix`
...
- keep Windows `.bat` lifecycle scripts as the authoritative operational contract for managed projects in `v0.3.24`
