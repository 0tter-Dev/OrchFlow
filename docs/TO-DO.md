...
1. **Version Bump Discipline Tests**
    Add validation that checks version-bearing files remain synchronized when a pull request advances the project version.

2. **Migration Validation Hardening**
    Add deeper migration checks when justified by the current Alembic model.

3. **Release Tag And Notes Automation**
    Add eventual release automation for tag validation and release notes.
...
- keep Windows `.bat` lifecycle scripts as the authoritative operational contract for managed projects in `v0.3.15`
