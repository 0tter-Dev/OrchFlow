...
At the current implementation stage, the AI assistance flow exposes safe status, LiteLLM gateway health, model discovery checks, authorized context manifests, reviewable analysis proposals, and API/CLI proposal reviews. Authenticated users can run `orchflow ai status --token <TOKEN>`, `orchflow ai health --token <TOKEN>`, `orchflow ai models --token <TOKEN>`, `orchflow ai manifest-create --token <TOKEN> --project-id <ID> --selected-model <MODEL> --intended-operation improve_lifecycle_script`, `orchflow ai manifest-show --token <TOKEN> --manifest-id <ID>`, `orchflow ai proposal-create --token <TOKEN> --manifest-id <ID>`, `orchflow ai proposal-show --token <TOKEN> --proposal-id <ID>`, and `orchflow ai proposal-review --token <TOKEN> --proposal-id <ID> --decision approved`, or call `GET /ai/status`, `GET /ai/gateway/health`, `GET /ai/models`, `POST /ai/context-manifests`, `GET /ai/context-manifests/{manifest_id}`, `POST /ai/analysis-proposals`, `GET /ai/analysis-proposals/{proposal_id}`, and `POST /ai/analysis-proposals/{proposal_id}/review`. Context manifests record authorization metadata without storing file contents. Proposal creation may send only manifest-approved context to LiteLLM, records audit events, and persists structured proposal output for review without writing `.bat` scripts or persisting suggested mappings yet. Proposal approval validates first-argument dispatch, required canonical actions, and proposed mapping consistency before recording the review decision.
