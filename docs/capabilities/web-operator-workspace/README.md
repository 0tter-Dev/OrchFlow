...
- accessible UI primitives for current web flows
- project details
- runtime metrics display
- stable authenticated health refresh feedback
- lifecycle controls
...
- visual complexity should remain secondary to operational clarity in `v0.3.37`; the web workspace should feel like a focused operator tool rather than a marketing or bootstrap page
...
- `TanStack Table` powers the focused operational project table; filtering and sorting remain client presentation behavior while authorization and canonical filtering stay in the API
...
- `i18next` and `react-i18next` provide the centralized `pt-BR` and `en-US` resources for selected workspace-shell and preference controls; browser-language detection never overrides the authenticated user's persisted locale
