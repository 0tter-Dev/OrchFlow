...
This project should not adopt a heavy Git Flow model in `v0.3.39`.
...
- patch increments, such as `0.3.4` to `0.3.5`, for fixes, small internal improvements, documentation refinements, test additions, and CI changes that do not redefine product scope
...
The supported frontend runtime for `v0.3.39` is Node.js `24` LTS, pinned for CI in `.node-version` at `24.21.0`. The selected package manager is pnpm `12.4.1`, pinned in `interface/web/package.json` and resolved by the Corepack bundled with that Node.js line without global shim writes. Corepack `0.34.6` was validated with the pin; a separate global Corepack installation is not part of the supported workflow. The supported toolchain is Vite `8.3.0`, `@vitejs/plugin-react` `6.1.1`, Vitest `5.0.0`, jsdom `30.0.1`, and TypeScript `6.0.3`; TypeScript 7 remains deferred until typescript-eslint supports it.
...
- Playwright Chromium installation and deterministic account-creation and project-registration browser workflows
...
The repository now has the backend and frontend quality baseline needed for `v0.3.39`.
