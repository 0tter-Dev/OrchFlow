...
| --- | --- |
| Before implementation | Reconcile every `active` and `review` plan with GitHub and local `main`; confirm the first eligible Roadmap item and its completed dependencies; record explicit approval in the plan; synchronize `main`; create the branch from that baseline. |
