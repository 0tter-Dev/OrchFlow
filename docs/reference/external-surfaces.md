...
- project update, project reload, project unlink, and lifecycle function mapping flows should be mirrored in CLI and API when implemented
...
- CLI and API both expose authentication, authenticated user preferences, project registry, non-AI project updates, project unlink, configured-action lifecycle execution, single and batch runtime inspection, filtered admin audit history visibility, admin user updates, project owner management, manual lifecycle configuration, project reload, and AI assistance workflows. The web interface consumes existing contracts for user preferences, project editing, project unlink, project configuration health, action gating, reload, mapping updates, batch runtime status, AI proposal review/application, and audit troubleshooting.
...
- requested batch runtime inspection is available through `POST /projects/runtime-inspections` and the mirrored CLI command `runtime inspect-many`; both reuse project visibility rules, deduplicate repeated project IDs, and preserve requested order
...
External surfaces now include API and CLI workflows for updating user preferences, including bounded `appearance_mode` and `accent_color` values, project metadata and lifecycle script paths, unlinking projects without deleting local files, manual mapping updates, explicit unconfigured-function decisions, explicit reload for one or more projects, AI-assisted improvement proposal workflows, and clear lifecycle execution rejection when an action is undefined, explicitly unconfigured, or part of a blocked project. The web operator surface now combines those contracts into a preference-aware guided operational readiness panel for selected projects and preserves backend failure details in contextual error notices. Planned refinements include broader web operator experience polish.
...
- authentication: `POST /auth/register`, `POST /auth/login`, `GET /auth/me`, `GET /auth/me/preferences`, `PATCH /auth/me/preferences`, `GET /auth/users`, `PATCH /auth/users/{user_id}`
...
- projects: `POST /projects`, `GET /projects`, `GET /projects/{project_id}`, `PATCH /projects/{project_id}`, `DELETE /projects/{project_id}`, `PATCH /projects/{project_id}/lifecycle-configuration`, `POST /projects/{project_id}/reload`, `POST /projects/reload`, `POST /projects/{project_id}/owners/{user_id}`, `DELETE /projects/{project_id}/owners/{user_id}`
- lifecycle and runtime: `POST /projects/{project_id}/lifecycle/{action}`, `GET /projects/{project_id}/runtime`, `POST /projects/runtime-inspections`
...
- authentication commands: `orchflow auth register`, `orchflow auth login`, `orchflow auth me`, `orchflow auth preferences`, `orchflow auth update-preferences`, `orchflow auth users`, `orchflow auth update-user`
- project commands: `orchflow project register`, `orchflow project list`, `orchflow project show`, `orchflow project update`, `orchflow project unlink`, `orchflow project configure-lifecycle`, `orchflow project reload`, `orchflow project reload-many`, `orchflow project add-owner`, `orchflow project remove-owner`
- lifecycle commands: `orchflow lifecycle status`, `orchflow lifecycle start`, `orchflow lifecycle stop`, `orchflow lifecycle restart`
- runtime commands: `orchflow runtime inspect`, `orchflow runtime inspect-many`
