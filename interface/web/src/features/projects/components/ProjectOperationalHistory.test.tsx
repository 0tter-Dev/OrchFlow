...
describe("ProjectOperationalHistory", () => {
  it("shows only the selected project's recent authorized outcomes", () => {
    render(<BrowserRouter><ProjectOperationalHistory canLoadAuditEvents events={[{ action: "lifecycle.start", actor_user_id: 1, created_at: "2026-09-20T00:00:00Z", details: "succeeded:true", id: 1, target_id: "7", target_type: "project" }, { action: "auth.login", actor_user_id: 1, created_at: "2026-09-20T00:00:00Z", details: null, id: 2, target_id: "7", target_type: "user" }, { action: "lifecycle.stop", actor_user_id: 1, created_at: "2026-09-20T00:00:00Z", details: "succeeded:false", id: 3, target_id: "8", target_type: "project" }]} projectId={7} /></BrowserRouter>);
    ...
    expect(screen.getByRole("link", { name: "Open full history" })).toHaveAttribute("href", "/activity?project_id=7");
  });
